...
class Pose:
    def __init__(self, vlcobj):
        ...
        self.vlc_obj = vlcobj
        self.delta_delay1, self.delta_delay2 = 0, 0
    ...
    def estimate(self):
        ...
        s1_w1 = self.s1_w1[:, np.newaxis].T
        s2_w2 = np.cos(self.w2 * self.t)
        s2_w2 = self.s2_w2[:, np.newaxis].T

        # after going through ADC at receiver
        r1_w1 = (self.vlc_obj.H[0][0] * np.cos(
            self.w1 * (self.t - delta_delay1)) + self.noise_standard_deviation * np.random.randn(1, len(self.t)))
        r2_w1 = (
        ...
        tx_pos = change_cords(np.array([[self.t_x_1, self.t_y_1], [self.t_x_2, self.t_y_2]]))
